...
public class ControlImage extends StyledText {

	private Image image;
	private boolean stayClosed = false;
	protected boolean showingEnlargedImage = false;

	public ControlImage(Composite parent, int style) {
		super(parent, style);
		initialize();
	}
	...
	/*
	 * private void showEnlargedImage() { if (showingEnlargedImage) return; if
	 * (stayClosed) return; if (image == null) return;
	 * 
	 * final Rectangle imageBounds = image.getBounds(); Rectangle canvasBounds =
	 * getBounds();
	 * 
	 * // Don't bother if the image is smaller than the canvas. if
	 * (imageBounds.width < canvasBounds.width & imageBounds.height <
	 * canvasBounds.height) return;
	 * 
	 * Rectangle displayBounds = getDisplay().getBounds(); int x =
	 * (canvasBounds.width - imageBounds.width) / 2; int y =
	 * (canvasBounds.height - imageBounds.height) / 2; Point where =
	 * toDisplay(new Point(x,y));
	 * 
	 * x = Math.max(0, Math.min(where.x,
	 * displayBounds.width-imageBounds.width)); y = Math.max(0,
	 ...
	 * shell.addPaintListener(new PaintListener() { public void
	 * paintControl(PaintEvent e) { e.gc.drawImage(image, 1, 1);
	 ...
	 * mouseDown(MouseEvent e) { stayClosed = true; showingEnlargedImage =
	 * false; shell.close(); } }); shell.addListener(SWT.MouseExit, new
	 * Listener() { public void handleEvent(Event event) { showingEnlargedImage
	 * = false; shell.close(); } });
	 * 
	 * shell.open(); }
	 */
